...
export function toRoutePattern(pathname: string): string {
  ...
  // does not normally route it, but a stale service worker can answer the
  // navigation from its precache and land on SharedRoundPage instead. Named
  // explicitly rather than left to the isIdLike fallback: a share id is
  // base64url and can come out purely alphabetic, which that net does not catch
  // (see isIdLike).
